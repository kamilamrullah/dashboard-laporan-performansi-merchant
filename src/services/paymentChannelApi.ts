...
export async function fetchPaymentChannels(search: string, status: 'all' | 'active' | 'inactive', page: number, signal?: AbortSignal): Promise<PaymentChannelMasterList> {
  ...
  const response = await fetch(`/api/payment-channels.php?${parameters}`, { signal, headers: { Accept: 'application/json' } });
...
export async function createPaymentChannel(sicCode: string, channelName: string): Promise<void> {
  const response = await fetch('/api/payment-channels.php', { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'create', sic_code: sicCode, channel_name: channelName }) });
...
export async function setPaymentChannelActive(sicCode: string, isActive: boolean): Promise<void> {
  const response = await fetch('/api/payment-channels.php', { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'set_active', sic_code: sicCode, is_active: isActive }) });
