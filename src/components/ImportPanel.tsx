...
import { clearTransactionPreviewToken, confirmTransactionImport, fetchTransactionPreviewRows, getTransactionPreviewToken, previewTransactionImport, storeTransactionPreviewToken } from '../services/transactionImportApi';
import type { MerchantOption, TransactionImportBatchDetail, TransactionImportOutcome, TransactionImportPreview, TransactionImportResult } from '../types';
...
export function ImportPanel({ type = 'transactions', merchantOptions = [], onCompleted, onPendingPreviewChange }: ImportPanelProps) {
  ...
  const createPreview = async (selectedFile: File) => {
    ...
    try {
      setPreviewFilter('');
      const created = await previewTransactionImport(selectedFile, selectedMerchant?.id ?? null, isNewMerchant ? merchantInput.trim() : null);
      storeTransactionPreviewToken(created.batch_id, created.confirmation_token);
      setPreview(created);
    }
  ...
  // Membangun kembali preview dari batch history menggunakan token yang masih tersimpan di sesi browser.
  const resumePreview = async (detail: TransactionImportBatchDetail) => {
    const token = getTransactionPreviewToken(detail.batch.id);
    if (!token) { setError('Token preview tidak tersedia pada sesi browser ini. Upload ulang file untuk melanjutkan.'); setActiveView('upload'); return; }
    const restored: TransactionImportPreview = {
      status: 'PREVIEWED', batch_id: detail.batch.id, original_filename: detail.batch.original_filename,
      confirmation_token: token, confirmation_expires_at: detail.batch.confirmation_expires_at ?? '',
      period_start: detail.batch.detected_period_start, period_end: detail.batch.detected_period_end,
      summary: detail.summary, rows: [], pagination: detail.rows.pagination,
    };
    setIsLoadingPreviewPage(true); setError(null); setPreviewFilter('');
    try {
      const page = await fetchTransactionPreviewRows(restored, 1, '');
      setPreview({ ...restored, rows: page.items, pagination: page.pagination }); setResult(null); setActiveView('upload');
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Preview tidak dapat dilanjutkan.'); setActiveView('upload'); }
    finally { setIsLoadingPreviewPage(false); }
  };
  ...
  if (activeView === 'history') return <div className="space-y-5">{viewTabs}<TransactionImportHistory onResume={(detail) => void resumePreview(detail)}/></div>;
