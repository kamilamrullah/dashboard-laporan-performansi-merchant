...
import { clearTransactionPreviewToken, deleteTransactionPreview, fetchTransactionImportBatch, fetchTransactionImportHistory, getTransactionPreviewToken } from '../services/transactionImportApi';
...
function BatchDetail({ batchId, onBack, onResume, onDeleted }: { batchId: number; onBack: () => void; onResume: (detail: TransactionImportBatchDetail) => void; onDeleted: () => void }) {
  ...
  const token = getTransactionPreviewToken(batch.id);
  const tokenActive = batch.status === 'PREVIEWED' && token !== null && batch.confirmation_expires_at !== null && new Date(batch.confirmation_expires_at.replace(' ', 'T')).getTime() >= Date.now();

  // Menghapus preview setelah konfirmasi eksplisit dan membersihkan token sesi lokal.
  const removePreview = async () => {
    if (!token) return;
    setIsDeleting(true); setError(null);
    try { await deleteTransactionPreview(batch.id, token); clearTransactionPreviewToken(batch.id); onDeleted(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'Preview gagal dihapus.'); setShowDeleteConfirmation(false); }
    finally { setIsDeleting(false); }
  };

  return <div className="space-y-4"><button onClick={onBack} className="inline-flex items-center gap-2 text-xs font-bold text-indigo-600"><ArrowLeft className="h-4 w-4"/>Kembali ke riwayat</button><section className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row"><div><div className="flex items-center gap-2"><h3 className="text-sm font-bold text-slate-900">Batch #{batch.id}</h3><span className={`rounded-full px-2 py-1 text-[9px] font-bold ${statusStyles[batch.status] ?? 'bg-slate-100 text-slate-600'}`}>{batch.status}</span></div><p className="mt-1 text-xs font-semibold text-slate-700">{batch.original_filename}</p><p className="mt-1 text-[10px] text-slate-400">{batch.merchant_name ?? 'Merchant tidak tersedia'} · {batch.detected_period_start ?? '—'} sampai {batch.detected_period_end ?? '—'} · dibuat {formatDateTime(batch.created_at)}</p>{batch.status === 'PREVIEWED' && !tokenActive && <p className="mt-2 text-[10px] font-semibold text-amber-700">Token tidak tersedia atau kedaluwarsa. Upload ulang file untuk melanjutkan.</p>}</div><div className="grid grid-cols-4 gap-2 text-center">{[['Insert', batch.inserted_rows], ['Update', batch.updated_rows], ['Duplikat', batch.duplicate_rows], ['Ditolak', batch.rejected_rows]].map(([label, value]) => <div key={label} className="rounded-lg bg-slate-50 px-3 py-2"><p className="text-sm font-bold text-slate-900">{value}</p><p className="text-[9px] uppercase text-slate-400">{label}</p></div>)}</div></div>{tokenActive && <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-4"><button onClick={() => onResume(detail)} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-bold text-white"><Play className="h-3.5 w-3.5"/>Lanjutkan Import</button><button onClick={() => setShowDeleteConfirmation(true)} className="inline-flex items-center gap-2 rounded-xl border border-rose-200 px-4 py-2.5 text-xs font-bold text-rose-700"><Trash2 className="h-3.5 w-3.5"/>Hapus Preview</button></div>}{showDeleteConfirmation && <div className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-4"><p className="text-xs font-bold text-rose-900">Hapus preview {batch.original_filename}?</p><p className="mt-1 text-[10px] leading-4 text-rose-700">Staging dan audit preview yang belum dikonfirmasi akan dihapus. Tindakan ini tidak menyentuh transaksi completed.</p><div className="mt-3 flex gap-2"><button disabled={isDeleting} onClick={() => setShowDeleteConfirmation(false)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[10px] font-bold text-slate-600">Batal</button><button disabled={isDeleting} onClick={() => void removePreview()} className="inline-flex items-center gap-2 rounded-lg bg-rose-600 px-3 py-2 text-[10px] font-bold text-white disabled:opacity-50">{isDeleting && <LoaderCircle className="h-3 w-3 animate-spin"/>}Ya, hapus preview</button></div></div>}</section>{error && <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs text-rose-700">{error}</div>}<div className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="overflow-x-auto scrollbar-subtle"><table className="min-w-[1050px] w-full text-left text-xs"><thead className="bg-slate-50 text-[10px] uppercase text-slate-500"><tr><th className="px-4 py-3">Baris</th><th className="px-4 py-3">Hasil</th><th className="px-4 py-3">Tanggal</th><th className="px-4 py-3">Tipe</th><th className="px-4 py-3">Partner</th><th className="px-4 py-3">SIC / RC</th><th className="px-4 py-3 text-right">Total trx</th><th className="px-4 py-3 text-right">Nominal</th></tr></thead><tbody className="divide-y divide-slate-100">{detail.rows.items.map((row) => { const data = row.normalized_data; const old = row.existing_data; return <tr key={row.id}><td className="px-4 py-3 font-mono text-slate-500">{row.source_row_number}</td><td className="px-4 py-3"><span className="rounded-full bg-slate-100 px-2 py-1 text-[9px] font-bold text-slate-700">{row.outcome}</span>{row.validation_errors?.message && <p className="mt-2 max-w-56 text-[10px] text-rose-600">{row.validation_errors.message}</p>}</td><td className="px-4 py-3">{data?.transaction_date ?? '—'}</td><td className="px-4 py-3 font-semibold">{data?.transaction_type ?? '—'}</td><td className="px-4 py-3">{data?.partner_channel ?? '—'}</td><td className="px-4 py-3 font-mono text-[10px]">{data ? `${data.sic_code} / ${data.response_code}` : '—'}</td><td className="px-4 py-3 text-right">{old && old.total_trx !== data?.total_trx && <span className="mr-2 text-[10px] text-slate-400 line-through">{formatAuditNumber(old.total_trx)}</span>}{formatAuditNumber(data?.total_trx)}</td><td className="px-4 py-3 text-right">{old && old.total_amount !== data?.total_amount && <span className="mr-2 text-[10px] text-slate-400 line-through">{formatAuditNumber(old.total_amount)}</span>}{formatAuditNumber(data?.total_amount)}</td></tr>; })}{detail.rows.items.length === 0 && <tr><td colSpan={8} className="px-4 py-10 text-center text-slate-400">Batch belum memiliki audit baris.</td></tr>}</tbody></table></div><Pagination page={detail.rows.pagination.page} totalPages={detail.rows.pagination.total_pages} onChange={setPage}/></div></div>;
}

// Menampilkan daftar batch import transaksi dan membuka detail audit batch yang dipilih.
export function TransactionImportHistory({ onResume }: { onResume: (detail: TransactionImportBatchDetail) => void }) {
  const [page, setPage] = useState(1);
  ...
  if (selectedBatch !== null) return <BatchDetail batchId={selectedBatch} onBack={() => setSelectedBatch(null)} onResume={onResume} onDeleted={() => { setSelectedBatch(null); refresh(); }}/>;
